...
class ResNet50(nn.Module): # inherit from nn.Module
    def __init__(self, num_classes, loss={'xent'}, **kwargs):
        ...
        self.classifier = nn.Linear(2048, num_classes) # input=2048, ouput = 751
        self.feat_dim = 2048 # feature dimension

    def forward(self, x):
        x = self.base(x) # output is [32, 2048, 8, 4]
        #embed()
        x = F.avg_pool2d(x, x.size()[2:]) # kernel_size = [8,4], map a (8x4) feature map into a unit, output is [32,2048,1,1]
        f = x.view(x.size(0), -1) # reshape into [32,2048] dimension
